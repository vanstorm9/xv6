...
int cowfork(void){
  ...
  // Copy process state from p.
  if((np->pgdir = duplicateuvm(proc->pgdir, proc->sz)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
    return -1;
...
void signal_deliver(int signum, siginfo_t sig_i)
{
	...
	*((uint*)(proc->tf->esp - 20)) = (uint) sig_i.type;
	*((uint*)(proc->tf->esp - 24)) = (uint) sig_i.addr;
